...
gulp.task('css', function(){
  return gulp.src([
		...
		.pipe(minifyCSS())
		.pipe(gulp.dest(distPath + '/assets/css/forum/'))
		.pipe(livereload());
});
...
gulp.task('css-theme', function(){
	...
	var tasks = colorList.map(function (color) {
		return gulp.src([ 'sass/theme.scss' ])
			...
			.pipe(minifyCSS())
			.pipe(gulp.dest(distPath + '/assets/css/forum/theme/'));
  });
...
gulp.task('js', function(){
  return gulp.src([
    ...
    .pipe(gulp.dest(distPath + '/assets/js/forum/'))
    .pipe(livereload())
});
...
gulp.task('default', gulp.series(gulp.parallel(['fonts', 'css', 'css-theme', 'js', 'plugins', 'webserver', 'watch'])));