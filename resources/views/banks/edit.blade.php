...
<div>
	<div class="panel panel-inverse" data-sortable-id="ui-widget-1">
	  	...
	  	<div class="panel-body">
	        ...
	        @endif
		    <form method="Post" action="{{url('banks/'.$bank->id)}}" class="form-group">
		    	@csrf
		    	@method('PUT')
		    	<fieldset>
		    		<legend class="m-b-10">{{$bank->name}}<small>Edit</small></legend>
		    		<div class="row">
		    			<div class="col" id="col-1">
							...
							<div class="form-group row m-b-15">
							  	...
							  	<div class="col-md-9">
								    <input type="text" name="short_code" class="form-control" value="{{$bank->short_code}}" />
								 </div>
							</div>
							<div class="form-group row m-b-15">
								...
								<div class="col-md-9">
							        <select name="country_id" class="border shadow p-2 bg-white form-control col-md-3" >
							            ...
							            <option @if($bank->country_id == $country->id) selected="selected" @endif value="{{$country->id}}">{{$country->name}}</option>
							            @endforeach
							        </select>
							    </div>
							</div>
